...
function makeOneMovieHtmlEl(newMovieObj) {
  ...
  const liEl = document.createElement("li");
  liEl.className = "movie-element";
  // prisideti data-movie-id atributa, kad atskirti indvidualu li el
  liEl.dataset.movieId = newMovieObj.id;
  // vidinius elementus su string (veliau reiktu perdaryti i createElement)
  const liInsideHtml = `
  <div class="movie-element__image">
...
function movieDeleteHandler(event) {
  ...
  const idOdElToBeDeleted = movieElToDelete.dataset.movieId;
  console.log("idOdElToBeDeleted ===", idOdElToBeDeleted);

  // mainMoviesArr paliekam ir istrinam visk isskyrus ta el ant kurio paspaudem delete
  mainMoviesArr = mainMoviesArr.filter((mObj) => mObj.id !== idOdElToBeDeleted);
